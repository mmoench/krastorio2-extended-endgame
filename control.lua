...
script.on_init(function()
    ...
    storage.kee_open_guis = {}                  -- List of all currently open guis
end)
...
function on_entity_created(event)
    ...
    if entity.name == "kee-intergalactic-transceiver-loading" then -- Only the loading entity should get created by the player
        ...
        local struct = {
            ...
            combinator                      = nil,    -- the entity of the hidden combinator
            state                           = 'loading',
            completed_activations           = 0,

            -- Stores the current amount of resources inserted into the "loading" entity:
...
script.on_event(defines.events.on_tick, function(event)
    ...
    for index, struct in pairs(storage.kee_intergalactic_transceivers) do
        ...
        -- In case we modified one of the transceivers, it is safer to quite the for-loop:
